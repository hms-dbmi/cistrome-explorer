		...
		if (maxDistance) {
			...
			if (isLeft) {
				...
				pathFunction = d => {
					return two.makePath(
						visWidth - d.parent.y,
						top + d.parent.x + titleHeight + 30,
						visWidth - d.parent.y,
						top + d.x + titleHeight + 30,
						visWidth - d.y,
						top + d.x + titleHeight + 30,
						visWidth - d.parent.y,
						top + d.x + titleHeight + 30
		...
		d3.select(canvas).on('mousemove', () => {
			...
			if (cannotAlign) {
				...
				PubSub.publish(EVENT.TOOLTIP, {
					x: mouseViewportX,
					y: mouseViewportY,
					content: <TooltipContent title={'Right-click to view highlight and filter options.'} />
				});
	...
	return (
		...
		>
			...
			{cannotAlign ? (
				...
				>
					...
					<span style={{ fontStyle: 'italic', fontWeight: 'normal' }}>
						{rowAggregated ? (
							<>
								To see the dendrogram, you need to first deactivate{' '}
								<span style={{ color: '#2299DB' }}>Aggregate By Tissue</span>.
							</>
						) : (
					...
					{!rowAggregated ? (
						<div
							onClick={() => onSortRows(field, type, 'ascending')}
							style={{
								position: 'absolute',
						...
						>
							<svg className={'hm-button-lg'} viewBox={SORT_TREE.viewBox}>
			...
			{isShowControlButtons && highlightNodeX && highlightNodeY ? (
				...
				>
					<circle r="6" fill="gray" stroke="none" opacity="0.6" cx={highlightNodeX} cy={highlightNodeY} />
				</svg>
			) : null}
			{minSimBar}
		</div>
