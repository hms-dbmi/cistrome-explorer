...
export default function RangeSlider(props) {
	const { isRight, height, valueExtent, onChange, onFilter, onClose } = props;
	...
	const moverSize = 16;
	const xScale = d3.scaleLinear().domain([leftMost, rightMost]).range([0, sliderWidth]);
	...
	useEffect(() => {
		...
		d3.select(leftMover).on('mouseenter', () => {
			selectedMover.current = 'left';
		});
		d3.select(rightMover).on('mouseenter', () => {
			selectedMover.current = 'right';
		});
	...
	return (
		<div className="hm-range-slider-container">
			<input
				ref={minInputRef}
