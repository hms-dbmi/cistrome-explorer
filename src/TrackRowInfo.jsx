...
import TrackRowInfoVis from './TrackRowInfoVis.jsx';
...
export default function TrackRowInfo(props) {
	...
	const [trackWidths, setTrackWidths] = useState(
		new Array(
			getNumOfTracks(
				isLeft,
				rowInfoAttributes.map(d => d.resolveYScale)
			)
		)
	);
	...
	function generateTrackProps() {
		...
		let currentLeft = 0,
			prevTrackSelectedRows = selectedRows;
		...
		rowInfoAttributes.forEach((attribute, i) => {
			...
			if (resolveYScale && order) {
				...
				currSelectedRows = selectRows(originalRowInfo, { rowFilter }, { rowSort: [{ field, type, order }] });
				currTransformedRowInfo = !currSelectedRows
					? originalRowInfo
					: currSelectedRows.map(indexOrIndices =>
	...
	const setUnitWidthByIndex = useCallback(
		(i, val) => {
