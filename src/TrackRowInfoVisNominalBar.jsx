...
export default function TrackRowInfoVisNominalBar(props) {
	...
	const colorScale = useMemo(
		() =>
			d3
				.scaleOrdinal()
				.domain(categories ?? Array.from(new Set(rowInfo.map(d => aggValue(d)))).sort())
	...
	const draw = useCallback(domElement => {
		...
		transformedRowInfo.forEach((d, i) => {
			...
			if (i + 1 < transformedRowInfo.length && category === aggValue(transformedRowInfo[i + 1])) {
	...
	useEffect(() => {
		...
		d3.select(canvas).on('mousemove', () => {
			...
			PubSub.publish(EVENT.TOOLTIP, {
				x: mouseViewportX,
				y: mouseViewportY,
				content: <TooltipContent title={title} value={hoveredCategory} color={colorScale(hoveredCategory)} />
			});
		});
