...
export default function TrackRowFilter(props) {
	const { isLeft, top, left, field, type, aggFunction, onChange, onFilterRows, onClose, rowInfo, filterInfo } = props;
	...
	const [valueExtent, setValueExtent] = useState(
		d3.extent(rowInfo.map(d => getAggregatedValue(d, field, type, aggFunction)))
	);
	...
	function onResetClick() {
		if (type === 'nominal' || type == 'link') {
			onFilterRows(
				field,
				type,
				rowInfo.map(d => getAggregatedValue(d, field, type, aggFunction).toString()),
				true
			);
	...
	return (
		...
		>
			...
			>
				...
				{type === 'quantitative' ? (
					<svg className="hm-button-sm" onClick={onFilterClick} viewBox={FILTER.viewBox}>
						<title>Filter rows by the range of values</title>
						<path d={FILTER.path} fill="currentColor" />
					</svg>
				) : null}
				<svg
					className="hm-button-sm"
					onClick={onResetClick}
					viewBox={type === 'quantitative' ? RESET.viewBox : SQUARE_CHECK.viewBox}
				>
					<title>Remove all filters</title>
					<path d={type === 'quantitative' ? RESET.path : SQUARE_CHECK.path} fill="currentColor" />
				</svg>
				{type === 'nominal' || type === 'link' ? (
					<svg className="hm-button-sm" onClick={onUnckeckAllClick} viewBox={SQUARE.viewBox}>
						<title>Unckeck all categories</title>
						<path d={SQUARE.path} fill="currentColor" />
					</svg>
				) : null}
				<svg className="hm-button-sm" onClick={onFilterClose} viewBox={CLOSE.viewBox}>
					...
					<ul>
						{suggestions.map((d, i) => (
							<li
								key={d}
								className={
									'hm-filter-suggestion-text ' + (i === suggestionIndex ? 'active-suggestion' : '')
								}
