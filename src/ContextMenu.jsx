...
export default function ContextMenu() {
	...
	function ContextMenuItem(props) {
		const { isSeparator, key, icon, title, action } = props;

		return isSeparator ? (
			<hr className="hm-context-menu-separator" key={key} />
		) : (
			<div
				className={action ? 'hm-context-menu-item' : 'hm-context-menu-item-title'}
				key={key}
				onClick={action}
				style={{
					display: 'flex',
			...
			>
				{icon ? (
					<svg className="hm-button-sm hm-button-static" viewBox={icon.viewBox}>
	...
	return (
		<div
			className="hm-context-menu-container"
			style={{
