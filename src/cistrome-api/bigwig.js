	...
	class CistromeBigWigDataFetcherClass {
		...
		loadChromsizes(dataConfig) {
			if (dataConfig.chromSizesUrl) {
				...
				console.error('Please enter a "chromSizesUrl" field to the data config');
			}
			return null;
		}
		...
		tile(z, x) {
			return this.tilesetInfo().then(tsInfo => {
				...
				const query = (c, s, e, cs) => {
					return fetch(
						`http://develop.cistrome.org/cistrome/samples/${this.dataConfig.cid}/track?chrom=${c}&start_pos=${s}&end_pos=${e}&bin_length=${basesPerBin}`
					)
