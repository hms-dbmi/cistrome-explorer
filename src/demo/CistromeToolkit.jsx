...
export default function CistromeToolkit(props) {
	const { isVisible: initIsVisible, onAddTrack, intervalAPIParams, geneAPIParams, hmRef, isMiraData } = props;
	...
	function runCistromeToolkitAPI(apiType, parameter = undefined) {
		...
		if (parameter) {
			...
			addRequestHistory(apiType, parameter, [], []);
			requestDBToolkitAPI(apiType, parameter)
				.then(([rows]) => {
					...
					addRequestHistory(apiType, parameter, customColumns, customRows);
				})
				.catch(msg => {
	...
	const dragged = useCallback(() => {
		...
		setHeight(Math.max(Math.min(height - diff, window.innerHeight), 400));
	}, [dragY, height]);
	...
	const APIConfigurationViews = useMemo(() => {
		const searchButton = (isReady, onClick) => (
			<div className={isReady ? 'api-search-button' : 'api-search-button-disabled'} onClick={onClick}>
				<svg
					className="hm-button-sm hm-button-static"
				...
				PubSub.publish(EVENT.TOOLTIP, {
					...
					content: (
						<TooltipContent
							...
							value={
								'This is the relative distance that the distribution of RP calculation function is decreased to half of the highest value at TSS'
							}
						/>
					)
				});
				...
				PubSub.publish(EVENT.TOOLTIP, {
					...
					content: (
						<TooltipContent
							title={'Bed File'}
							value={
								'Three columns are required (i.e., chromosome, start site, end site), and tab delimited, maximum of 50,000 peaks'
							}
						/>
					)
				});
		...
		return (
			<>
				{/* Search by Interval */}
				<div className={'api-config-view'} style={{ borderLeft: `4px solid ${CISTROME_API_COLORS.INTERVAL}` }}>
					...
					<input
						...
						onChange={e =>
							setLatestIntervalParams({ ...latestIntervalParams, chrStartPos: e.target.value })
						}
					/>
					...
					/>
					{searchButton(isLatestIntervalParamsReady, () =>
						runCistromeToolkitAPI(CISTROME_API_TYPES.INTERVAL)
					)}
				</div>
				{/* Search By Gene */}
				<div className={'api-config-view'} style={{ borderLeft: `4px solid ${CISTROME_API_COLORS.GENE}` }}>
						...
						onKeyDown={e => {
							switch (e.key) {
								...
								case 'Enter':
									...
									setLatestGeneParams({
										...latestGeneParams,
										gene: geneSuggestions[geneIndex].geneName
									});
									break;
								case 'Esc':
								case 'Escape':
					...
					{geneSuggestions.length !== 0 ? (
						<div
							className="gene-suggestion"
							style={{
								position: 'fixed',
								left: geneSearchRef.current?.getBoundingClientRect().left,
								top:
									geneSearchRef.current?.getBoundingClientRect().top +
									geneSearchRef.current?.getBoundingClientRect().height
							}}
						>
							<ul>
								{geneSuggestions.map((d, i) => (
									<li
										style={{
											textAlign: 'right',
											color: 'gray',
											background: i === geneIndex ? '#E2F1FF' : 'none'
										}}
				...
				{/* Search By Factor */}
				<div className={'api-config-view'} style={{ borderLeft: `4px solid ${CISTROME_API_COLORS.FACTOR}` }}>
					...
					<input
						...
						onChange={e => {
							...
							if (bedFile) {
								...
								reader.onload = () => {
									setLatestPeaksetParams({
										...latestPeaksetParams,
										bedFile: new Blob([reader.result])
									});
								};
			...
			if (api === CISTROME_API_TYPES.INTERVAL) {
				...
				return (
					...
					>
						...
						<div className="cisvis-api-parameter">
							<span>{'INTERVAL'}</span>
							<b>{`${chrStartName}:${chrStartPos.toLocaleString('en')}-${chrEndPos.toLocaleString(
								'en'
							)}`}</b>
						</div>
					</div>
			...
			<div className="cisvis-data-table-container">
				<h4 className="cisvis-table-title">
					<span style={{ position: 'relative', top: 5 }}>
						<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox={TABLE.viewBox}>
				...
				<span className="cisvis-table-subtitle">
					{requestStatus?.isLoading && requestStatus?.msg ? <b>{requestStatus?.msg}</b> : null}
					&nbsp;&nbsp;
					{requestStatus?.isLoading ? <span className="cisvis-progress-ring" /> : null}
				</span>
				<span
					style={{
				...
				<div className="cisvis-cistrome-toolkit-body">
					...
					<div className="cisvis-api-request">{APIConfigurationViews}</div>
					<div className="cisvis-api-result-list">{listOfResultsRequested}</div>
					<DataTable
