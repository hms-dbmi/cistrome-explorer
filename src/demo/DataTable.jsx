...
export default function DataTable(props) {
	const { rows = [], columns = [], onButton = null, expoNotations = [] } = props;
	...
	const bodyRows = useMemo(() => {
		...
		const dataRows = transformedRows.slice(0, ROW_DISPLAY_LIMIT).map((d, i) => {
			const buttonCell = onButton ? (
				<td>
					<span style={{ position: 'relative', top: 3 }} onClick={() => onButton(transformedRows[i])}>
						<svg
							className="hm-button"
	...
	return (
		<>
			...
			>
				...
				>
					{`Showing the first ${
						transformedRows.length < ROW_DISPLAY_LIMIT ? transformedRows.length : ROW_DISPLAY_LIMIT
