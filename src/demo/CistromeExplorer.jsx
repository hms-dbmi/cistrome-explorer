...
import CistromeToolkit from './CistromeToolkit.jsx';
...
export default function CistromeExplorer() {
	...
	const onAddToolkitTrack = useCallback(
		(cid, gsm, ct, f) => {
	...
	useEffect(() => {
		function closeSideViews(e) {
			if ((e.key === 'Esc' || e.key === 'Escape') && (isSettingVisible || isToolkitVisible)) {
		...
		<div className="cistrome-explorer">
			<div className="header-container">
				<div className="header">
					<span className="ce-generic-button" onClick={() => setIsSettingVisible(!isSettingVisible)}>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox={MENU.viewBox}>
							<title>Menu</title>
					...
					<span className="header-control">
						...
						{'Data '}
						<select onChange={e => setSelectedDemo(e.target.value)} defaultValue={selectedDemo}>
							{Object.keys(demos).map(vcKey => (
								<option key={vcKey} value={vcKey}>
									{vcKey}
								</option>
							))}
						</select>
					...
					>
						<span className="ce-generic-button" style={{ cursor: 'auto' }}>
							<svg xmlns="http://www.w3.org/2000/svg" viewBox={SEARCH.viewBox}>
								<path d={SEARCH.path} fill="currentColor" />
							</svg>
							<input
								ref={searchBoxRef}
								...
								onChange={e => {
									...
									if (keyword !== '' && !keyword.startsWith('c')) {
										...
										setSuggestionPosition({
											...
											top:
												searchBoxRef.current.getBoundingClientRect().top +
					...
					>
						...
						>
							<svg xmlns="http://www.w3.org/2000/svg" viewBox={FOLDER.viewBox}>
					...
					>
						...
						>
							<svg xmlns="http://www.w3.org/2000/svg" viewBox={TABLE_2.viewBox}>
					...
					>
						<span
							className={
								'ce-generic-button-lg ' +
								(aggActivated ? 'ce-generic-button-activated ' : '') +
								(helpActivated ? 'help-highlight' : '')
							}
							style={{ cursor: 'auto' }}
						>
							<svg
			...
			<div className="visualization-container">
				...
				>
					...
					>
						<svg style={{ color: 'rgb(171, 171, 171)', width: 14, height: 14 }} viewBox={UNDO.viewBox}>
							<title>Undo</title>
					...
					>
						<svg style={{ color: 'rgb(171, 171, 171)', width: 14, height: 14 }} viewBox={REDO.viewBox}>
							<title>Redo</title>
					...
					<span
						...
						onClick={() => {
							setHelpActivated(!helpActivated);
						}}
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
					...
					<span
						...
						onClick={() => {}}
					>
						<svg style={{ color: 'rgb(171, 171, 171)', width: 14, height: 14 }} viewBox={SEARCH.viewBox}>
					...
					>
						<svg style={{ color: 'rgb(171, 171, 171)', width: 14, height: 14 }} viewBox={TRASH.viewBox}>
							<title>Open Metadata</title>
					...
					>
						<svg style={{ color: 'rgb(171, 171, 171)', width: 14, height: 14 }} viewBox={FOLDER.viewBox}>
							<title>Open Metadata</title>
					...
					<span
						...
						onClick={() => {}}
					>
						<svg style={{ color: 'rgb(171, 171, 171)', width: 14, height: 14 }} viewBox={PENCIL.viewBox}>
					...
					>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox={DOCUMENT.viewBox}>
					...
					>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox={GITHUB.viewBox}>
							<title>GitHub</title>
