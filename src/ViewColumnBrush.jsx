...
export default function ViewColumnBrush(props) {
	const { viewBoundingBox, viewportTrack, multivecTrack, onViewportRemove, onRequestIntervalTFs } = props;
	...
	useEffect(() => {
		...
		const absDomain = viewportTrack.viewportXDomain;
		resolveIntervalCoordinates(assembly, absDomain[0], absDomain[1]).then(result => {
			if (!didUnmount) {
	...
	const { msg: intervalInvalidMsg, success: intervalValid } = validateIntervalParams({ assembly, ...positionInfo });
	...
	return (
		...
		>
			{end - start > minWidthToShowButton ? (
				<div
					className={'hm-button-sm-container-horizontal'}
					style={{
						right: '0px',
						height: '100%',
						opacity: 1,
				...
				>
					...
					{onRequestIntervalTFs ? (
						...
						<svg
							className={'hm-button-sm hm-button-middle'}
							style={{ height: '100%' }}
							onMouseOver={e =>
								setOnSearchButton({
									on: true,
