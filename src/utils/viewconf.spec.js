...
describe('Utilities for processing higlass view config objects', () => {
	...
	it('Should set the selectRows option value of a particular horizontal-multivec track', () => {
		...
		const newViewConfig = updateViewConfigOnSelectRowsByTrack(
			currViewConfig,
			[1, 2, 3],
			'cistrome-view-2',
			'cistrome-track-2'
	...
	it('Should get viewId and trackId pairs of horizontal-multivec tracks', () => {
		...
		expect(
			searchedPairs.find(d => d.viewId === 'cistrome-view-4-1' && d.trackId === 'cistrome-track-4-1') !==
				undefined
		).toEqual(true);
	});
	...
	it('Should get viewId and trackId pairs of all kinds of tracks', () => {
		...
		expect(
			searchedPairs.find(d => d.viewId === 'cistrome-view-4-2' && d.trackId === 'conservation-track-2') !==
				undefined
		).toEqual(true);
	});
	...
	it('Should remove a viewport-projection-horizontal track from viewConfig', () => {
		const updatedViewConfig = removeViewportFromViewConfig(
			hgDemoViewConfig3,
			'cistrome-view-1',
			'cistrome-view-1-projection-track-1'
		);
