		...
		fieldInfo: {
			...
			properties: {
				...
				shortName: {
					type: 'string',
					description:
						'A short version of an alternative name to display in axis label elements when they are assigned to narrow space.'
				},
				addTrackOnClick: {
					type: 'boolean',
				...
				title: {
					type: 'string',
					description:
						'A human-readable title of the data field to display in axis titles and tooltip elements.'
				},
				type: {
					type: 'string',
				...
				resolveYScale: {
					type: 'boolean',
					description:
						'Determine if the scale of y axis should be independent to the adjacently placed tracks'
				},
				sort: {
					type: 'string',
				...
				domain: {
					...
					items: { type: 'string' },
					description:
						"Only applicable with type === 'nominal-dynamic'. Set the domain for the nominal field, for example ['positive', 'negative']."
				},
				range: {
					type: 'array',
					items: { type: 'string' },
					description:
						"Only applicable with type === 'nominal-dynamic'. Set the color range for the nominal field, for example ['blue', 'red']."
				}
			}
		},
		sortInfo: {
...
export function getConditionFromHighlightOption(highlitOption) {
	...
	const key = Object.keys(highlitOption).find(
		k => k !== 'field' && k !== 'type' && k === getHighlightKeyByFieldType(type, highlitOption[k]) // key should match with the given field type
	);
