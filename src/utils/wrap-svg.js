...
export function wrapSvg(hgSvgNode, drawInfo) {
	...
	const maxX = Math.max(
		hgOffsetX + hgWidth,
		...Object.values(drawInfo).map(d =>
			d.options && d.options.left && d.options.width ? hgOffsetX + d.options.left + d.options.width : 0
		)
	);
	const maxY = hgHeight;

	// Set the height and width of the outer SVG.
	svg.attr('width', maxX).attr('height', maxY).attr('xmlns', 'http://www.w3.org/2000/svg').attr('version', '1.1');
	...
	for (const [key, d] of Object.entries(drawInfo)) {
		...
		const tempSvg = d3.create('svg').attr('width', width).attr('height', height);
