...
export default class Two {
	...
	initSvg() {
		...
		this.g = this.svg.append('g').attr('width', this.width).attr('height', this.height);
	}
	...
	updateSvg() {
		...
		this.elements.forEach(d => {
			if (d instanceof TwoRectangle) {
				...
				if (d.stroke != null) {
					rect.attr('stroke-width', d.linewidth).attr('stroke', d.stroke);
				}
				if (d.rotation != null) {
					rect.attr(
						'transform',
						`rotate(${(d.rotation * 180) / Math.PI},${d.x + d.width / 2},${d.y + d.height / 2})`
					);
				}
				...
				if (d.stroke != null) {
					circle.attr('stroke-width', d.linewidth).attr('stroke', d.stroke);
				}
				...
				const line = g
					...
					.attr('opacity', d.opacity);
				if (d.stroke != null) {
					line.attr('stroke-width', d.linewidth).attr('stroke', d.stroke);
				}
			} else if (d instanceof TwoPath) {
				const path = g.append('path').attr('opacity', d.opacity);
				...
				if (d.fill != null) {
					path.attr('fill', d.fill);
				} else {
					path.attr('fill', 'transparent');
				}
				if (d.stroke != null) {
					path.attr('stroke-width', d.linewidth).attr('stroke', d.stroke);
				}
				path.attr('d', pathD);
				...
				if (d.rotation != null) {
					text.attr('transform', `rotate(${(d.rotation * 180) / Math.PI},${d.x},${d.y})`);
				}
			}
		});
	}
