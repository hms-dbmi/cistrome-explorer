...
export function makeDBToolkitGeneAPIURL(assembly, gene, distance) {
	...
	return `http://dbtoolkit.cistrome.org/api_gene?species=${assembly}&factor=tf&transcript=${gene}&distance=${distance.replace(
		'b',
		''
	)}`;
}
...
export function requestByInterval({ assembly, chrStartName, chrStartPos, chrEndName, chrEndPos }) {
	const { msg: validationMsg, success: validationSuccess } = validateIntervalParams({
		assembly,
		chrStartName,
		chrStartPos,
		chrEndName,
		...
		.then(data => {
			return new Promise((resolve, reject) => {
				...
				const filteredRows = rows
					.filter(d => d.species !== 'mus_musculus')
					.slice(0, rows.length < SEARCH_ITEM_LIMIT ? rows.length : SEARCH_ITEM_LIMIT);
