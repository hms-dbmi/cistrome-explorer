...
import TrackWrapper from './TrackWrapper.jsx';
import ViewWrapper from './ViewWrapper.jsx';
import Tooltip from './Tooltip.jsx';
import ContextMenu, { destroyContextMenu } from './ContextMenu.jsx';
import GwasFilter from './GwasFilter.jsx';
...
const HiGlassMetaConsumer = forwardRef((props, ref) => {
	...
	const setMetadataToContext = useCallback(
		(viewId, trackId) => {
	...
	const getTrackObject = useCallback(
		(viewId, trackId) => {
			try {
	...
	const getViewBoundingBox = useCallback(
		viewId => {
			try {
				const viewConfig = hgRef.current.api.getViewConfig();
				const trackIds = getAllViewAndTrackPairs(viewConfig, { onlyHorizontalMultivec: false }).filter(
					d => d.viewId === viewId
				);
	...
	useEffect(() => {
		...
		const newViewConfig = setDataTransformOfTopTrackFromViewConfig(
			...
			[
				'cistrome-view-1',
				'cistrome-view-atac',
				'cistrome-view-atac-revision',
				'cistrome-view-3k27',
				'cistrome-view-3k27-revision',
				'cistrome-track-3k4',
	...
	const setSelectedRowsToViewConfig = useCallback(
		(viewId, trackId, selectedRows) => {
			const currViewConfig = hgRef.current.api.getViewConfig();
	...
	const onSortRows = useCallback(
		(viewId, trackId, field, type, order, isTrackIndependent) => {
			let newOptions;
			if (isTrackIndependent) {
				...
				newOptions = updateWrapperOptions(options, newRowInfoAttributes, 'rowInfoAttributes', viewId, trackId, {
					isReplace: true
				});
			} else {
	...
	const onHighlightRows = useCallback(
		(viewId, trackId, field, type, condition) => {
			...
			const newOptions = updateWrapperOptions(options, newRowHighlight, 'rowHighlight', viewId, trackId, {
				isReplace: true
			});
			setOptions(newOptions);
		},
	...
	useEffect(() => {
		...
		const newOptions = updateWrapperOptions(options, newRowAggregate, 'rowAggregate', viewId, trackId, {
			isReplace: true
		});
		setOptions(newOptions);
	}, [aggregateRowBy]);
	...
	const onFilterRows = useCallback(
		(viewId, trackId, field, type, condition, isRemove) => {
			...
			if (isRemove) {
				...
				if (type === 'nominal' || type === 'link') {
					...
					if (fieldOption) {
						...
						newSubOptions = modifyItemInArray(newSubOptions, fieldOptionIndex, {
							...fieldOption,
							[key]: condition // We do not want to remove a `ancestors` or `minSimilarity` option if exists.
						});
					} else {
			...
			let newOptions = updateWrapperOptions(options, newSubOptions, 'rowFilter', viewId, trackId, {
				isReplace: true
			});
	...
	const onZoomRows = useCallback(
		(viewId, trackId, y, deltaY, deltaMode) => {
			...
			const oldRowZoom = oldWrapperOptions.rowZoom || {
				level: 1.0,
				top: 0.0,
				numRows: trackNumRowsSelected || trackNumRowsTotal
			};
			...
			const newWrapperOptions = updateWrapperOptions(options, newRowZoom, 'rowZoom', viewId, trackId, {
				isReplace: true
			});
	...
	const onAddTrack = useCallback(
		(viewId, trackId, field, type, notOneOf, position, selected = '') => {
			...
			// Add filter options.
			newOptions = updateWrapperOptions(newOptions, newRowFilter, 'rowFilter', viewId, newTrackId, {
				isReplace: false
			});
	...
	const hgComponent = useMemo(() => {
		...
		return <HiGlassComponent viewConfig={baseViewConfig} options={hgOptions} zoomFixed={false} ref={hgRef} />;
		...
		<div className="hm-root">
			...
			{stackedBarTrackIds.map(({ viewId, trackId }, i) => {
				...
				return (
					<div
						key={i}
						style={{ left: left + w - 20, top: top + 3, position: 'absolute', height: h }}
			...
			{gwasTrackIds.map(({ viewId, trackId }, i) => {
				...
				return (
					<div
						key={i}
						style={{ left: left + w - 20, top: top, position: 'absolute', height: h }}
			...
			{Array.from(new Set(multivecTrackIds.map(d => d.viewId))).map((viewId, i) => (
				<ViewWrapper
					...
					viewportTracks={
						viewportTrackIds[viewId]
							? viewportTrackIds[viewId].map(d => getTrackObject(viewId, d.trackId))
							: []
					}
					...
					onSelectGenomicInterval={(startProp, endProp, uid) => {
						const currViewConfig = hgRef.current.api.getViewConfig();
						const newViewConfig = updateViewConfigOnSelectGenomicInterval(
							currViewConfig,
							viewId,
							startProp,
							endProp,
